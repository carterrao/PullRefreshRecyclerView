...
import com.rzc.pullrefreshrecyclerviewlib.R;
...
public class RefreshRecyclerView extends RecyclerView {
    public static final int HOLDER_TYPE_REFRESH = 100001;
    public static final int HOLDER_TYPE_LOAD_MORE = 100002;
    ...
    private void init(Context context) {
        ...
        mRefreshRecyclerViewAdapter = new RefreshRecyclerViewAdapter(this);
        super.setAdapter(mRefreshRecyclerViewAdapter);
        ...
        mGridLayoutManager.setSpanSizeLookup(new GridLayoutManager.SpanSizeLookup() {
            ...
            public int getSpanSize(int position) {
                int pos = 1;
                for (int i = 0; i < mRefreshRecyclerViewAdapter.subAdapterList.size(); i++) {
                    RefreshRecyclerViewBaseSubAdapter subAdapter = mRefreshRecyclerViewAdapter.subAdapterList.get(i);
                    int subCount = subAdapter.getItemCount();
                    int subPosition = position - pos;
                    if (subPosition < subCount) {
                        int spanCount = (int) (mSpanCount / subAdapter.getColumnCount()
    ...
    public void addSubAdapter(RefreshRecyclerViewBaseSubAdapter adapter) {
        if (mRefreshRecyclerViewAdapter.addSubAdapter(adapter)) {
            mSpanCount = leastCommonMultiple(mSpanCount, adapter.getColumnCount());
            mGridLayoutManager.setSpanCount(mSpanCount);
            mRefreshRecyclerViewAdapter.notifyDataSetChanged();
        }
    }

    public void insertSubAdapterBefore(RefreshRecyclerViewBaseSubAdapter adapter, RefreshRecyclerViewBaseSubAdapter beforeAdapter) {
        if (mRefreshRecyclerViewAdapter.insertSubAdapter(adapter, beforeAdapter, true)) {
            mSpanCount = leastCommonMultiple(mSpanCount, adapter.getColumnCount());
            mGridLayoutManager.setSpanCount(mSpanCount);
            mRefreshRecyclerViewAdapter.notifyDataSetChanged();
        }
    ...
    public void insertSubAdapterAfter(RefreshRecyclerViewBaseSubAdapter adapter, RefreshRecyclerViewBaseSubAdapter beforeAdapter) {
        if (mRefreshRecyclerViewAdapter.insertSubAdapter(adapter, beforeAdapter, false)) {
            mSpanCount = leastCommonMultiple(mSpanCount, adapter.getColumnCount());
            mGridLayoutManager.setSpanCount(mSpanCount);
            mRefreshRecyclerViewAdapter.notifyDataSetChanged();
        }
    ...
    public void removeSubAdapter(RefreshRecyclerViewBaseSubAdapter adapter) {
        if (mRefreshRecyclerViewAdapter.removeSubAdapter(adapter)) {
            mRefreshRecyclerViewAdapter.notifyDataSetChanged();
        }
    }
